USE Sakila;
select count(*) AS `Copies Hunchback Impossible`
FROM inventory
...
Select count(*) AS `Copies Hunchback Impossible`
FROM inventory
JOIN (select title, film_id FROM film WHERE title = 'Hunchback Impossible') as film_filtered ON film_filtered.film_id=inventory.film_id;

Select AVG(length) FROM film;

select * from film WHERE lenth> (Select AVG(length) FROM film);
